...
\end{minted}
on accède une fois à chaque case \visible<2>{$\rightarrow$ \textbf{complexité $O(n)$}}
\end{exampleblock}
\end{frame}
...
\end{minted}
deux parcours imbriqués \visible<2>{$\rightarrow$ complexité $O(n^2)$}
\end{exampleblock}
\end{frame}
...
Les problèmes de classe $P$ sont les problèmes que l'on peut résoudre en \textbf{temps polynomial}.\\
Les problèmes de classe $NP$ sont ceux pour lesquels on \textbf{ne connait pas} de solution en temps polynomial.
\end{block}
...
\end{minipage}
\visible<3->{Approche naïve : $O(n!)$, meilleur algorithme exact connu : $O(n^2 2^n)$.}
\end{exampleblock}
}
