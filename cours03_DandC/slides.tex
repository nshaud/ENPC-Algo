...
\begin{frame}{QuickSort : complexité}
 ...
 $$ {\color{blue} C_{moy}(N)} = {\color{red} N - 1 } + {\color{green!50!black} {\color{brown} \frac{1}{N} \sum_{{\color{purple} p=1}}^{{\color{purple} N}}}  C_{moy}({\color{purple} p-1}) + C_{moy}({\color{purple} N-p})}~~.$$
 }
\end{frame}
...
\begin{frame}{QuickSort : complexité}
 ...
 Comme :
 $$\color{red} \sum_{k=1}^{N} \frac{1}{k}\sim_{N \rightarrow \infty} log(N) $$

\uncover<2>{
 il vient\,:

 $${\color{blue} C_{moy}(N)} \sim_{N \rightarrow \infty} (N+1) {\color{red} log(N)} + (N+1) {\color{yellow!50!black} \frac{C_{moy}(1)}{2}}$$
}
 Finalement\,:
\begin{block}<3>{}
 $$C_{moy}(N) = O(N \log(N))$$
...
$$ {\color{blue} C_N} = {\color{red} N} + {\color{green!50!black} 2* C_{\frac{N}{2}}}$$}
\uncover<3->{Au rang suivant :
$$ {\color{blue} C_N} = {\color{red} 2N} + {\color{green!50!black} 4* C_{\frac{N}{4}}}$$}
\uncover<4->{Puis :
$$ {\color{blue} C_N} = {\color{red} k N} + {\color{green!50!black} 2^k * C_{\frac{N}{2^k}}}$$}
\uncover<5->{La récurrence se termine après $k = log_2(N)$ étapes, donc\,:
$$C_N = N \log N + N C_1 = O(N \log N)$$}
\end{frame}
...
$$DFT_{paire}[k + \frac{N}{2}] = DFT_{paire}[k],$$
$$DFT_{impaire}[k + \frac{N}{2}] = DFT_{impaire}[k]~~.$$
}
\only<3>{
\begin{enumerate}
\item Calculer la DFT des indices pairs de $f$.
